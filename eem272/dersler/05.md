...
[Sınıflandırma slaytı için tıklayınız.](./slides/ders05.pdf)   
...
Veri dosyasını indirmek için [tıklayınız](./images/05_insurance_data.csv).


```python
df = pd.read_csv("05_insurance_data.csv")
df.head()
```
...
```python
def f(x, theta0=theta0, theta1=theta1):
